...
export default class ServerlessAssumeRole {
              ...
              apply: async (target, thisArg, argumentsList) => {
                if (!assumed) {
                  ...
                  if (Credentials === undefined) {
                    throw new Error(
                      'Failed to get credentials from assume role request'
                    );
                  }
                  /*
                  ...
                  utils.log.notice(
                    `Assume role succeeded! The new credentials is valid until ${Expiration.toLocaleString()}`
                  );
  ...
  private getAssumeRoleParams(): AWS.STS.Types.AssumeRoleRequest {
    const {
      ...
      this.serverless,
      ['service', 'custom', 'assumeRole', 'params'],
      {}
    );
    ...
    return {
      DurationSeconds:
        durationSeconds === undefined ? undefined : intDurationSeconds,
